...
app.controller("RDataController", ["$scope", "$http", "$q", "$location", "$httpParamSerializer", "$interval", "$window",
    ...
    // sortmanager init => Location Change Success Event
    // console.log( ZIMT.DATA.Session.langMgr.getLang() );
...
//        rxData.sync2amodl();
    ...
    this.onResetForm = function() {
      ...
      ZIMT.DATA.Session.trvisMgr.reset();
 ...
 */
    $scope.$on('$locationChangeSuccess', function() {
        ...
        ZIMT.DATA.Session.sortMgr.init( ZIMT.DATA.Session.langMgr.getLang() ); // hmm warum hier?
