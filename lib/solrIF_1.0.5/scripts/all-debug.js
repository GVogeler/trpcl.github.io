...
ZIMT.UTILS.TimeRangeVisibilityManager.prototype.reset = function() {
    this.setHide();
}
...
ZIMT.UTILS.PaginationManager = function() {
	...
	this.__mode = this.enumMode.FIRST_CAP;
}
...
ZIMT.UTILS.PaginationManager.prototype.getCurrPag = function() {
	...
	var __sp = {page:"..", link:"..", nr:0, class:"spacer"};

	if(this.__isSinglePage()) __result.pagvis = false;
	
	if(this.__mode === this.enumMode.FIRST_CAP) {
		__result.pag.push( __begin );
		this.__getNumberItems( __result.pag );
		__result.pag.push( __sp );
		__result.pag.push( __end );
	}
	else if (this.__mode === this.enumMode.FIRST_ITEM)
	{
	    this.__getNumberItems( __result.pag );
	    __result.pag.push( __sp );
        __result.pag.push( __end );
	}
	else if (this.__mode === this.enumMode.LAST_CAP)
	{
		__result.pag.push( __begin );
		__result.pag.push( __sp );
		this.__getNumberItems( __result.pag );
	...
	else if (this.__mode === this.enumMode.LAST_ITEM)
    {
        __result.pag.push( __begin );
        __result.pag.push( __sp );
        this.__getNumberItems( __result.pag );
    ...
    else if (this.__mode === this.enumMode.SHORT_FIRST)
    {
        this.__getNumberItems( __result.pag );
        __result.pag.push( __end );
    }
    else if (this.__mode === this.enumMode.SHORT_LAST)
    {
        __result.pag.push( __begin );
        this.__getNumberItems( __result.pag );
    }
...
ZIMT.UTILS.PaginationManager.prototype.__getNumberItems = function( arr ) {
	...
	if(this.__mode === this.enumMode.FIRST_CAP ||
	   this.__mode === this.enumMode.FIRST_ITEM)
	{
		for (var i = 0; i < this.__nrPages; i++) {
			...
			if ((i+1)>=this.__nrVisPages) break;
	         ...
	         this.__mode === this.enumMode.LAST_ITEM)
	{
	...
	else if (this.__mode === this.enumMode.IN)
	{
		...
		for (var i = start; i < this.__nrPages; i++) {
			...
			if ((i+1)>=(this.__nrVisPages+start)) break;
	         ...
	         this.__mode === this.enumMode.SHORT_FIRST ||
	         this.__mode === this.enumMode.SHORT_LAST)
	{
		for (var i = 0; i < this.__nrPages; i++) {
			arr.push( this.__getItemText(i) );
		}
...
ZIMT.UTILS.PaginationManager.prototype.decideType = function() {
	if (this.__nrPages <= this.__nrVisPages)
	{
	    ...
	    else if ((1 + this.__currentPage) == this.__nrPages) {
	        this.__mode = this.enumMode.SHORT_LAST;
	    }
	    else {
	...
	{
	    if ((1 + this.__currentPage) == this.__nrPages)
	    {
	        this.__mode = this.enumMode.LAST_ITEM;
	    } else
	    {
	        this.__mode = this.enumMode.LAST_CAP;
	    }
	}
	else {
...
ZIMT.UTILS.SortManager = function( arrSA ) {
	this.__list = {};
	this.__arrSA = arrSA;
	this.__lang = ""; // collation

	this.__langPoss = {
	    en: "_collen",
	    de: "_collger",
	    da: "_collda"  // nur zum testen
	};
...
ZIMT.UTILS.SortManager.prototype.init = function(  tmpLang ) {
	var i = 0;

	this.__lang = this.checkLanguage( tmpLang );
//	console.log( "init Sort Manager: " + this.__lang );
 ...
 * set Serachable
 * 
 * @param itemSA  name and type of sa z.B.: ["dc_contributor", "str"]
 * 
 * @retruns false if item exists
 */

ZIMT.UTILS.SortManager.prototype.setSA = function( itemSAarr ) {
    var itemSA = itemSAarr[0];
	...
	if (this.__isItemSA( itemSA )) {
		...
		this.__list[itemSA]["type"] = itemSAarr[1];
		return true;
	}
}
...
ZIMT.UTILS.SortManager.prototype.getType = function( iName) {
    return this.__list[iName].type;
}

ZIMT.UTILS.SortManager.prototype.getSortFieldName = function( iName ) {
    var __tmp = iName;

    // str
    if("str" == this.getType( iName )) {
        __tmp += this.__langPoss[this.__lang];
    }

    // nr
    // im falle von nr ist derzeit nichts nötig denke ich.
...
ZIMT.UTILS.SortManager.prototype.getSortQuery = function() {
	...
	for (i = 0; i < __arrTmp.length; i++) {
		...
		{
		    // Oje, das geht nur gut solange nur nach einer ausprägung sortiert wird.
		    // Sonst fehlt hier der Beistrich!
		    __tmp += this.getSortFieldName( __arrTmp[i] )
			    + " "
			    + this.__list[__arrTmp[i]].order;
			__bYesWeSort = true;
		}
	}
 ...
 * get Sort
 * 
 * nur zum schreiben in die location, nicht in die query.
 */
...
/**
 * 
 * @param tmp languagestring
 * 
 * @returns
 */
ZIMT.UTILS.SortManager.prototype.checkLanguage = function( tmp ) {

    if ( this.__langPoss.hasOwnProperty( tmp ) )
    {
        return tmp;
    } else
    {
        console.log( "Error: unconfigured language in use." );
        return "de";
    }
...
ZIMT.UTILS.Lang.prototype.getLang = function() {
    return this.__lang;
}
